    ...
    <main className="auth-page">
      ...
      <div className="container-page auth-page-grid py-12">
        <section className="auth-brand-panel fade-in-up">
          <BrandLogo brandName={t("common.brandName")} size="hero" showName={false} animated />
        ...
        <Card className="auth-card fade-in-up fade-in-up--delay">
          <div className="mb-6 flex flex-col items-center text-center">
            <BrandLogo brandName={t("common.brandName")} size="md" showName={false} />
            <h2 className="mt-4 text-3xl font-bold">{t("auth.loginTitle")}</h2>
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{t("auth.loginDescription")}</p>
          </div>
          {session ? (
            <Link href={`/${locale}/account`} className="inline-flex font-bold text-[var(--brand)]">
              {t("nav.account")}
            </Link>
          ) : (
